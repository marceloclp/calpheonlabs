...
const ItemExchangeByNpcPayload = struct({
    ...
    requiredItemCount: u64().transform((value) => value.toString()),
    /** Item granted by the recipe. */
    rewardItemId: u32(),
    /** Exact unsigned quantity granted, retained as a decimal string. */
    rewardItemCount: u64().transform((value) => value.toString()),
}).fixedLength(26);
...
const ItemExchangeByNpcRow = struct({
    ...
    .check((row) => row.outerExchangeId === row.payload.exchangeId)
    .fixedLength(28);
